...
- **Watch mode uses first-trigger-immediate debounce.** After informer cache sync, `runLeader` enqueues an initial trigger so zero-CRD clusters can still publish optional schemas. The informer's initial List can also fire AddFunc for existing CRDs; the one-slot trigger channel coalesces duplicates. The first trigger fires immediately (zero delay), subsequent triggers are debounced.
- **Watch mode uses full re-extract.** Simpler than incremental. Upload is already incremental via check-missing. Extraction of <200 CRDs is sub-second. `SCHEMA_FILTER_*` and `--kind`/`--group`/`--version` filter CRD and built-in output for each cycle, while the informer still watches all CRDs. If built-ins are enabled, `/openapi/v2` is refreshed on each publish cycle. If an active filter matches no CRDs or built-ins and Kustomize is not enabled, runtime builds create and publish an empty catalog so stale broader output is not preserved.
...
- **Helm chart** in `charts/crd-schema-publisher/` distributed as OCI artifact via GHCR. Two modes (`controller`/`cronjob`) with mode-isolated templates. Two-tier optional secret management: `existingSecret`, `externalSecret` (ESO). Both controller and CronJob modes gracefully degrade to extract-only without Cloudflare creds: controller runs `watch` and omits the publisher, while CronJob runs the default `run` command and skips upload after extraction. `config.filter.kind`/`group`/`version` map to `SCHEMA_FILTER_KIND`/`GROUP`/`VERSION` when non-empty. `config.includeBuiltins` maps to `SCHEMA_INCLUDE_BUILTINS` and adds `/openapi/v2` ClusterRole permission only when both it and `rbac.create` are true. `config.includeKustomize` maps to `SCHEMA_INCLUDE_KUSTOMIZE` and requires no additional Kubernetes permission. `values.schema.json` enforces secret mutual exclusivity via `if/then` (not `oneOf`, which breaks yaml-language-server with `additionalProperties: false`); template precedence in `_helpers.tpl` handles runtime resolution (`existingSecret.name` wins, else chart fullname). NetworkPolicy and CiliumNetworkPolicy are mutually exclusive (enforced via `fail` guard in `_helpers.tpl`). PrometheusRule only renders in controller mode. Chart version is CalVer SemVer: `YYYY.MDD.HMMSS` with no leading zeros in numeric components — e.g. `2026.413.65435`. Image and chart are always released together with the same CalVer version — both `version` and `appVersion` match the image tag. `Chart.yaml` version/appVersion fields are placeholder `0.0.0` — both overridden by the release workflow at package time. Dashboard embedded via `.Files.Get`. Pod anti-affinity presets in `_helpers.tpl`.
